...
export default function DocenteCalendarioPage() {
    ...
    const [activePlan, setActivePlan] = useState('diario');
    ...
    const canViewDiario = user?.jornada === 'diario' || user?.jornada === 'ambas';
    const canViewFinSemana = user?.jornada === 'fin_de_semana' || user?.jornada === 'ambas';

    useEffect(() => {
        // Establecer plan inicial según jornada
        if (user?.jornada === 'fin_de_semana') {
            setActivePlan('fin_de_semana');
        } else {
            setActivePlan('diario');
        }
    }, [user]);
            ...
            >
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-3">
                        ...
                        <div className="mb-6">
                            ...
                            <p className="text-gray-600">
                                Jornada: {user?.jornada === 'diario' && 'Diaria'}
                                {user?.jornada === 'fin_de_semana' && 'Fin de Semana'}
                                {user?.jornada === 'ambas' && 'Ambas Jornadas'}
                            </p>
                        </div>
                        ...
                        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
                            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                                {/* Selector de Plan (solo si tiene acceso a ambas jornadas) */}
                                {user?.jornada === 'ambas' && (
                                    <div className="flex items-center space-x-2">
                                        <label className="text-sm font-medium text-gray-700">Calendario:</label>
                                        <select
                                            value={activePlan}
                                            onChange={(e) => setActivePlan(e.target.value)}
                                            className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                                        >
                                            {canViewDiario && <option value="diario">Plan Diario</option>}
                                            {canViewFinSemana && (
                    ...
                    <div className="lg:col-span-1">
                        <UpcomingActivities plan={activePlan} rol="docente" />
                    </div>
                </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                        <div className="bg-white rounded-lg shadow-xl max-w-lg w-full">
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                {selectedActivity.dirigido_a === 'solo_docentes' && (
                                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                                        <p className="text-sm text-purple-700 font-medium">
                                            Esta actividad es exclusiva para docentes
                                        </p>
                                    </div>
                                )}
                            </div>
